...
public class Button extends Browser {
	...
	public void verifyButtonLinkContains(String attribute, String link) {
		element = Browser.findElement(by);
		if (element == null) {
			Log.testStep("FAILED", name + " Button is NOT displayed", name + " Button is displayed");
		} else {
			String x = element.getAttribute(attribute);
			if (x.equals(link)) {
				Log.testStep("PASSED", x + " link is equal to " + link, x + " link is equal to " + link);
			} else {
				Log.testStep("FAILED", x + " link is NOT equal to " + link, x + " link is equal to " + link);
			}
		}
	}
	...
	public void verifyButtonOpensLinkInNewTab(String link) throws InterruptedException {
		element = Browser.findElement(by);
		if (element == null) {
			Log.testStep("FAILED", name + " Button is NOT displayed", name + " Button is displayed");
		} else {
			...
			String openedURL = Browser.getDriver().getCurrentUrl().trim();

			if (openedURL.equals(link)){
				Log.testStep("PASSED", openedURL + " link is equal to " + link, openedURL + " link is equal to " + link);
			} else {
				Log.testStep("FAILED", openedURL + " link is NOT equal to " + link, openedURL + " link is equal to " + link);
			}
		}
	}

	public void verifyLinkOpensAndCloseTab(String link, Element checkerElement) {
		element = Browser.findElement(by);
		if (element == null) {
			Log.testStep("FAILED", name + " Button is NOT displayed", name + " Button is displayed");
		} else {
			ArrayList<String> tabs = new ArrayList<String> (Browser.getDriver().getWindowHandles());
			Browser.getDriver().switchTo().window(tabs.get(1));

			String openedURL = Browser.getDriver().getCurrentUrl().trim();
			if (openedURL.equals(link)){
				Log.testStep("PASSED", openedURL + " link is equal to " + link, openedURL + " link is equal to " + link);
				checkerElement.verifyDisplayed();
			} else {
				Log.testStep("FAILED", openedURL + " link is NOT equal to " + link, openedURL + " link is equal to " + link);
			}
