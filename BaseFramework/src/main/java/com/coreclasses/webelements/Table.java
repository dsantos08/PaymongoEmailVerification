...
public class Table extends Browser {
	...
	public Table(String name, By by) {
		this.by = by;
		this.name = name;
	}

	public List<WebElement> getAllSpanElements(){
		element = Browser.findElement(by);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("span")));
		return tableRows;
	}

	public List<WebElement> getAllAElements(){
		element = Browser.findElement(by);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("a")));
		return tableRows;
	}

	public List<WebElement> getAllIElements(){
		element = Browser.findElement(by);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("i")));
		return tableRows;
	}

	public List<WebElement> getAllH4Elements(){
		element = Browser.findElement(by);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("h4")));
		return tableRows;
	}
	...
	public void clickOnElementViaListJS(int i, List<WebElement> elements){
		element = elements.get(i);
		WebDriverWait wait = new WebDriverWait(Browser.getDriver(), 5);
		if (element == null) {
			Log.testStep("FAILED", name + " is NOT displayed", name + " is displayed");
	...
	public String getTextOfATableElement(int x, List<WebElement> tableElements){
		return tableElements.get(x).getText();
	}

	public List<WebElement> getAllButtons(){
		element = Browser.findElement(by);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("button")));
		return tableRows;
	}

	public void clickOptionsOfElementInTable(int title){
		element = Browser.findElement(by);
		WebDriverWait wait = new WebDriverWait(Browser.getDriver(), 5);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("a")));
		if (tableRows.get(title).isDisplayed()){
			Log.testStep("PASSED", tableRows.get(title).getText() + " is clicked", tableRows.get(title).getText() + " is clicked");
			tableRows.get(title).click();
		}
		else{
			Log.testStep("FAILED", tableRows.get(title).getText() + " is NOT present and cannot be clicked", tableRows.get(title).getText() + " is clicked");
		}
	}

	public void clickButtonInTable(int button){
		element = Browser.findElement(by);
		WebDriverWait wait = new WebDriverWait(Browser.getDriver(), 5);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("button")));
		if (tableRows.get(button).isDisplayed()){
			Log.testStep("PASSED", tableRows.get(button).getText() + " is clicked", tableRows.get(button).getText() + " is clicked");
			tableRows.get(button).click();
		}
		else{
			Log.testStep("FAILED", tableRows.get(button).getText() + " is NOT present and cannot be clicked", tableRows.get(button).getText() + " is clicked");
		}
	}

	public void clickFirstEnabledButton() {
		int button = 0;
		element = Browser.findElement(by);
		WebDriverWait wait = new WebDriverWait(Browser.getDriver(), 5);
		List<WebElement> tableRows = new WebDriverWait(Browser.getDriver(), 10).until(ExpectedConditions.visibilityOfNestedElementsLocatedBy(element, By.tagName("button")));
		if (tableRows.get(button).isDisplayed()) {
			while (tableRows.get(button).isEnabled() == false) {
				button++;
			}
			tableRows.get(button).click();
			Log.testStep("PASSED", tableRows.get(button).getText() + " is clicked", tableRows.get(button).getText() + " is clicked");
	...
	public void verifyContainsRowByString(String row, List<WebElement> tableElements){
		element = Browser.findElement(by);
		List<String> strings = new ArrayList<String>();
		for(WebElement e : tableElements){
			strings.add(e.getText());
		}
		boolean b = strings.contains(row);

		if (b)
			Log.testStep("PASSED", row + " is displayed in the table", row + " is displayed in the table");
		else
			Log.testStep("FAILED", row + " is NOT displayed in the table", row + " is displayed in the table");
	}
	...
	public void verifyTextContains(String text) {
		...
		if (element == null) {
			...
			if (element.getText().contains(text)) {
				Log.testStep("PASSED", name + " text contains " + element.getText(), name + " text contains " + text);
			} else {
				Log.testStep("FAILED", name + " text contains " + element.getText(), name + " text contains " + text);
			}
		}
	}
	...
	public void verifyTextEquals(String text) {
		...
		if (element == null) {
			...
			if (element.getText().equals(text)) {
				Log.testStep("PASSED", name + " text is equal to " + element.getText(), name + " text is equal to " + text);
			} else {
				Log.testStep("FAILED", name + " text is equal to " + element.getText(), name + " text is equal to " + text);
			}
		}
	}
