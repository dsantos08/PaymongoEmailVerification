...
public class TextBox extends Browser {
	...
	/*
	Gets the value of the TextBox via ID of the element in page source
	Issue is with text box not returning the value unless javascript command is used
	Will be used for the verification part of the content
	 */
	public String getJSTextByXPath(String xpath){
		element = Browser.findElement(by);
		String value;
		if (element == null) {
			return null;
		} else {
			value = jse.executeScript("return document.getElementByXpath('"+ xpath +"').value").toString();
			return value;
		}
	}

	public String getJSTextByName(String name) {
		element = Browser.findElement(by);
		String value;
		if (element == null) {
			return null;
		} else {
			value = jse.executeScript("return document.getElementsByName('"+ name +"')[0].value").toString();
			return value;
		}
	}

	public String getJSTextByID(String id){
		element = Browser.findElement(by);
		String value;
		if (element == null) {
			return null;
		} else {
			value = jse.executeScript("return document.getElementById('"+ id +"').value").toString();;
			return value;
		}
	}

	public String trimXpathToID(){
		String idOfElement = StringUtils.substringBetween(this.by.toString(), "'", "'");
		return idOfElement;
	}

	public void verifyJSTextByIDValueEquals(String text) {
		String idOfElement = StringUtils.substringBetween(this.by.toString(), "'", "'");
		String valueOfTextBox = getJSTextByID(idOfElement);
		if (valueOfTextBox == null) {
			Log.testStep("FAILED", name + " TextBox is NOT populated", name + " TextBox is populated");
		} else {
			if (valueOfTextBox.equals(text)) {
				Log.testStep("PASSED", name + " TextBox text is equal to " + valueOfTextBox,
						name + " TextBox text is equal to " + text);
			} else {
				Log.testStep("FAILED", name + " TextBox text is equal to " + valueOfTextBox,
						name + " TextBox text is equal to " + text);
			}
		}
	}

	public void verifyJSTextByNameValueEquals(String text) {
		String idOfElement = StringUtils.substringBetween(this.by.toString(), "'", "'");
		String valueOfTextBox = getJSTextByName(idOfElement);
		if (valueOfTextBox == null) {
			Log.testStep("FAILED", name + " TextBox is NOT populated", name + " TextBox is populated");
		} else {
			if (valueOfTextBox.equals(text)) {
	...
	public void verifyReadOnly(){
		...
		if ((element.getAttribute("readonly")).equals("true")){
			Log.testStep("PASSED", name + " TextBox is read only",
					name + " TextBox is read only");
		}
		else{

			Log.testStep("FAILED", name + " TextBox is not read only",
					name + " TextBox is read only");
		}
	}
	...
	public void verifyTextContains(String text) {
		...
		if (element == null) {
			...
			if (element.getText().trim().contains(text)) {
				...
				Log.testStep("FAILED", name + " TextBox text does not contains " + text,
						name + " TextBox text contains " + text);
			}
		}
	}
